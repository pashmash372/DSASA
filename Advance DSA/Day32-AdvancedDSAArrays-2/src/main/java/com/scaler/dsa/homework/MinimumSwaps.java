...
public class MinimumSwaps {
    public int solve(int[] A, int B) {
        int n = A.length;
        int cnt = 0;
        // count number of elements <= B  (smaller than B)
        for (int x : A)
            if (x <= B) cnt++;
        ...
        //pointer approach
        while (r < cnt) { // first window
            if (A[r] > B) x++;
            r++;
        }
        int ans = x;
        while (r < n) { // later windows
            if (A[r] > B) x++;
            if (A[l] > B) x--;
            ans = Math.min(ans, x);
            r++;
...
class MinimumSwaps1 {
    public int solve(ArrayList<Integer> A, int B) {
        int count = 0, noSwap = 0, max = 0;
        for (int i = 0; i < A.size(); i++) if (A.get(i) <= B) count++; // counting smaller than = B
        for (int i = 0; i < count; i++) if (A.get(i) <= B) noSwap++;
        max = Math.max(max, noSwap);
        int start = 0, end = count;
        while (end < A.size()) {
            if (A.get(start) <= B) noSwap--;
            if (A.get(end) <= B) noSwap++;
            max = Math.max(max, noSwap);
            start++;
            end++;
        }
