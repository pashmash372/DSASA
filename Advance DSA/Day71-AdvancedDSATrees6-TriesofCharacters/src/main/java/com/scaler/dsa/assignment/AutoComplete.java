...
/*Another Solution*/
/*Java solution using Tries and TreeMap*/

class Tries {
    private final HashMap<Character, Tries> m_next;
    private final SortedMap<Integer, Integer> m_weight;

    public Tries() {
        m_next = new HashMap<Character, Tries>();
        m_weight = new TreeMap<Integer, Integer>(Collections.reverseOrder());
    }
...
class AutoComplete1 {
    private static Tries insertIntoTries(Tries root, String[] dictionary, int[] weight) {
        for (int i = 0; i < dictionary.length; i++) {
            Tries temp = root;
            String word = dictionary[i];
            for (int j = 0; j < word.length(); j++) {
                if (temp.hasNext(word.charAt(j))) temp = temp.getNext(word.charAt(j));
                else temp = temp.setNext(word.charAt(j));
    ...
    private static void searchOnTries(Tries root, String[] dictionary, String[] search) {
        for (int i = 0; i < search.length; i++) {
            ...
            if (bIsFound) temp.autoComplete(dictionary);
        }
    }
    ...
    public static void main(String[] args) {
        ...
        while (Q-- > 0) {
            ...
            root = insertIntoTries(root, dictionary, weight);
